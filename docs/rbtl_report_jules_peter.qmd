...
Globally, 43% of the population lacks access to safe sanitation[@diarrhea], leading to the spread of diarrheal diseases and environmental contamination from untreated faecal sludge. While urine diversion dry toilets (UDDTs) offer a potential solution by separating nutrient-rich urine for fertilizer, the handling and potential pathogen reactivation in faeces remain challenges. Current practices like desiccation with additives often fail to completely eliminate pathogens, necessitating improved treatment methods.
...
```{r}
#| label: fig-1
#| fig-cap: "Datastructure to be used in the experiments" 
...
Data analysis and figures are created using R (Rstudio V.2025.05.0 Build 496).

An R-script for Loading the relevant data (01-data_download) another one cleans the data and prepare it for the analysis (02-data_cleaning.R). These R scripts are stored in the folder fungal-faecal-treatment/R

Data files being saved in the folder data processed with their corresponding name for the analysis as CSV files. (growth_speed_analysis and bacteria_analysis)
...
### Testing growth data for normal distribution
...
The shapiro test verifies the normal distribution for *C. comatus* and faecal isolate 1 while it shows violation of normal distribution for *P. ostreatus v. Floridae.*

#### Box plot and statistical tests

A box-plot shows the median growth speed of the different fungal species.
...
ANOVA - an Tukey post hoc analysis
...
No signifant difference between the growth speed of the three fungal species under assuming normal distribution of the data.
...
```{r}
#| label: fig-4
#| fig-cap: "Mean growth speed of P. ostreatus v. Floridae depending on inocuum substrate used"
...
The Shapiro test **(tbl xy)** and the Q-Q plots **(figxy)** are showing that the created log change data for E. coli is normal distributed. This normal distribution allows to use an ANOVA for the data analysis.

```{r}
#| label: fig-6
#| fig-cap: "q-q plot for testing normal distribution of log-change of Enterococcus"
...
#| echo: false
#| label: tbl-10
#| tbl-cap: "ANOVA for testing of significant differences in Enterococcus logchange in different fungal species and control"
...
```{r}
#| label: tbl-11
#| tbl-cap: "Tukey for pairwise comparison of Enterococcus log-change in different fungal species and control"
...
```{r}
#| label: fig-7
#| fig-cap: "Boxplot comparing log change for E. coli and Enterococcus between treatments"
...
In the created data set there are significant differences in E. coli log change as showed in **(tbl xy)** significant difference in Ecoli log change as shown in **(tbl xy)** the fungal species and the control treatment. The difference between Faecal isolate 1 and the control is not significant in the example data set. For enterococcus the species P ostreatsu is significant higher than the other three treatments. However the other three treatments are not significantly different in this example data set.

# Conclusions

The produced data structure and scripts can be used on the test data set, to visualize mean growth speed differences between species. With the Bacterial And would allow for analysis of growth speed analysis of fungal data and bacterial concentration change analysis on faeces. To test the data on the real created data will be a future challenge and adaptions have to be implemented depending on how the experiment setup will change.
